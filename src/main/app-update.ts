...
function handleUpdateOnMac(): void {
      ...
      (_e, response: DownloadAppUpdateApprovalRes) => {
        if (response && response === 1) {
          ...
          fail(
            ...
            'Latest version of the app already is available, but the user refused to perform the update.'
          )
        }
      }
    )
...
function handleUpdateOnOthers(): void {

...
export class CompletedPage extends HTMLElement {
  ...
  private async _load(): Promise<void> {
    ...
    const jobs = (all || []).filter((j) => j.status === 'completed')
  ...
  private _renderJobs(jobs: Array<Job>): void {
    ...
    for (const j of jobs) {
      ...
      jobItem.setAttribute('progress', '100%')

      this._jobsList.appendChild(jobItem)
    }
  }
}
if (!customElements.get('completed-page')) {
  customElements.define('completed-page', CompletedPage)
}
declare global {
